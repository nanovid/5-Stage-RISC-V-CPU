...
```bash
git clone ist-git@git.uwaterloo.ca:ece621-f25/project/v2sharda-pd6.git
```
...
We use the [rv32-benchmarks](https://git.uwaterloo.ca/ece621-f25/rv32-benchmarks) to test your design.
...
The [rv32-benchmarks](https://git.uwaterloo.ca/ece621-f25/rv32-benchmarks) have subroutines called `pass` and `fail`, which write status values to certain registers.
...
Ensure that the output trace is correct for all [rv32-benchmarks](https://git.uwaterloo.ca/ece621-f25/rv32-benchmarks).
